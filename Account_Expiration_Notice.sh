...
# Note that i needed to Keep Log files as well for the result every day. i also needed these Log files to ba in a simple formatted directory of Date
# Like YYYYMMDD so here is why i set the 'CurrentDate' Env virable.
...
cd $DIR1
# SSH to run the main DB Script on the DB Server from Web server
ssh oracle@DB1 <<'ENDSSH'sh /home/oracle/MY-DIRECTORY/MY-SCRIPT.sh  # Please Note the user is 'oracle' in our first SSH.
ENDSSH

# i Did not want to mess with any permissions of the users specially oracle users so i chose to just run the script via oracle.
...
# i needed to get the reuslt file and clean them into single lines of Emails using at sign (@). you see the issue was with sql plus when i used spool the result file also included the query itself.
